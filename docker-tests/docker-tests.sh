...
configure_environment() {
  ...
  case "${DISTRIBUTION}_${VERSION}" in
    ...
    'ubuntu_18.04'|'ubuntu_16.04'|'debian_8')
      run_opts=('--volume=/run' '--volume=/run/lock' '--volume=/tmp' '--volume=/sys/fs/cgroup:/sys/fs/cgroup:ro' '--cap-add=NET_ADMIN' '--cap-add=SYS_ADMIN' '--cap-add=SYS_RESOURCE' '--device=/dev/fuse:/dev/fuse' '--privileged=true')
